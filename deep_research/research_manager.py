...
class ResearchManager:
    ...
    async def clarify(self, query: str) -> ClarifyingQuestions:
        """Generate the questions shown before research begins."""
        result = await Runner.run(clarifier_agent, f"Initial research request: {query}")
    ...
    async def run(self, query: str, clarifications: str = ""):
        ...
        if clarifications.strip():
            research_brief = (
                ...
                f"Researcher preferences and clarifications:\n{clarifications}"
            )
